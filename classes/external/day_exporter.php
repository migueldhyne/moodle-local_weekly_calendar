...
class day_exporter extends exporter {
    ...
    protected function get_navigation() {
        ...
        // Retrieve the course id and the day's timestamp.
        $courseid = $this->calendar->courseid;
        $time = $this->calendar->time;

        // Calculate the previous and next dates (in timestamp).
        $prevtime = strtotime('-1 day', $time);
        ...
        $prevurl = new \moodle_url($baseurl, ['time' => $prevtime]);
        // Link to the next day.
        $nexturl = new \moodle_url($baseurl, ['time' => $nexttime]);

        // Title of the day displayed, formatted according to user preferences.
        $title = userdate($time, get_string('strftimedaydate'));

        return [
            'prev' => \html_writer::link($prevurl, get_string('dayprev', 'calendar')),
            'next' => \html_writer::link($nexturl, get_string('daynext', 'calendar')),
            'title' => $title,
        ];
    }
